...
namespace OpenTelemetry.Trace.Test
{
    ...
    public class SpanBuilderTest : IDisposable
    {
        ...
        private readonly TracerConfigurationOptions alwaysSampleTracerConfigurationOptions = new TracerConfigurationOptions(Samplers.AlwaysSample);
        ...
        {
            tracer = new Tracer(spanProcessor, alwaysSampleTracerConfigurationOptions);
        }

        [Fact]
        public void StartSpanNullParent()
        {
            var span = (Span) new SpanBuilder(SpanName, spanProcessor, alwaysSampleTracerConfigurationOptions, Resource.Empty)
        ...
        public void StartSpanLastParentWins3()
        {
            var spanContext =
                new SpanContext(
                    ...
                    ActivitySpanId.CreateRandom(),
                    ActivityTraceFlags.None);
            var rootSpan = (Span)new SpanBuilder(SpanName, spanProcessor, alwaysSampleTracerConfigurationOptions, Resource.Empty)
                .StartSpan();

            var childSpan = (Span)new SpanBuilder(SpanName, spanProcessor, alwaysSampleTracerConfigurationOptions, Resource.Empty)
                .SetParent(spanContext)
                .SetParent(rootSpan)
                .StartSpan();
        ...
        {
            var span = new SpanBuilder(SpanName, spanProcessor, alwaysSampleTracerConfigurationOptions, Resource.Empty)
                .SetSpanKind(SpanKind.Internal)
                .SetSampler(Samplers.NeverSample)
                .SetNoParent()
                .StartSpan();
        ...
        {
            var rootSpan = new SpanBuilder(SpanName, spanProcessor, alwaysSampleTracerConfigurationOptions, Resource.Empty)
                .SetSpanKind(SpanKind.Internal)
                .SetNoParent()
                .StartSpan();
        ...
        public void StartSpanInScopeOfCurrentActivity()
        {
            ...
            var childSpan = new SpanBuilder(SpanName, spanProcessor, alwaysSampleTracerConfigurationOptions, Resource.Empty)
                .SetSpanKind(SpanKind.Internal)
                .StartSpan();
        ...
        {
            var rootSpanUnsampled = new SpanBuilder(SpanName, spanProcessor, alwaysSampleTracerConfigurationOptions, Resource.Empty)
                .SetSpanKind(SpanKind.Internal)
                .SetSampler(Samplers.NeverSample)
                .SetNoParent()
                .StartSpan();
            ...
            var rootSpanSampled =
                new SpanBuilder(SpanName, spanProcessor, alwaysSampleTracerConfigurationOptions, Resource.Empty)
                    .SetSpanKind(SpanKind.Internal)
                    .SetSampler(Samplers.AlwaysSample)
                    .SetNoParent()
                    .StartSpan();
        ...
        public void StartRemoteChildSpan_WithProbabilitySamplerDefaultSampler()
        {
            ...
            var childSpan =
                new SpanBuilder(SpanName, spanProcessor, new TracerConfigurationOptions(ProbabilitySampler.Create(0.1)), Resource.Empty)
                    .SetSpanKind(SpanKind.Internal)
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new SpanBuilder(null, spanProcessor, alwaysSampleTracerConfigurationOptions, Resource.Empty));
            Assert.Throws<ArgumentNullException>(() => new SpanBuilder(SpanName, null, alwaysSampleTracerConfigurationOptions, Resource.Empty));
            Assert.Throws<ArgumentNullException>(() => new SpanBuilder(SpanName, spanProcessor, null, Resource.Empty));

            var spanBuilder = new SpanBuilder(SpanName, spanProcessor, alwaysSampleTracerConfigurationOptions, Resource.Empty);
