...
namespace OpenTelemetry.Tests
{
    ...
    internal static class AttributesExtensions
    {
        ...
        {
            var expectedValuePairs = expectedAttributes as KeyValuePair<string, object>[] ?? expectedAttributes.ToArray();
            var actualValuePairs = attributes as KeyValuePair<string, object>[] ?? attributes.ToArray();
            Assert.Equal(actualValuePairs.Count(), expectedValuePairs.Count());

            foreach (var attr in actualValuePairs)
            {
                Assert.Contains(attr, expectedValuePairs);
            }
        }
    }
}
