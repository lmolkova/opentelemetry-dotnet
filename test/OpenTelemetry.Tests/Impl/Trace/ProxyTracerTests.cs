...
namespace OpenTelemetry.Tests.Impl.Trace
{
    public class ProxyTracerTests
    {
        ...
        public void ProxyTracer_CreateSpan_BadArgs()
        {
            ...
            Assert.Throws<ArgumentNullException>(() => proxyTracer.StartRootSpan(null, SpanKind.Client, default, null as IEnumerable<Link>, false));
            Assert.Throws<ArgumentNullException>(() => proxyTracer.StartRootSpan(null, SpanKind.Client, default, null as Func<IEnumerable<Link>>, false));
            ...
            Assert.Throws<ArgumentNullException>(() => proxyTracer.StartSpan(null, SpanKind.Client, default, null as IEnumerable<Link>, false));
            Assert.Throws<ArgumentNullException>(() => proxyTracer.StartSpan(null, SpanKind.Client, default, null as Func<IEnumerable<Link>>, false));
            ...
            Assert.Throws<ArgumentNullException>(() => proxyTracer.StartSpan(null, BlankSpan.Instance, SpanKind.Client, default, null as IEnumerable<Link>, false));
            Assert.Throws<ArgumentNullException>(() => proxyTracer.StartSpan(null, BlankSpan.Instance, SpanKind.Client, default, null as Func<IEnumerable<Link>>, false));
            ...
            Assert.Throws<ArgumentNullException>(() => proxyTracer.StartSpan(null, SpanContext.Blank, SpanKind.Client, default, null as IEnumerable<Link>, false));
            Assert.Throws<ArgumentNullException>(() => proxyTracer.StartSpan(null, SpanContext.Blank, SpanKind.Client, default, null as Func<IEnumerable<Link>>, false));
        ...
        public void ProxyTracer_CreateSpan_ValidArgs()
        {
            ...
            Assert.Equal(BlankSpan.Instance, proxyTracer.StartRootSpan("foo", SpanKind.Client, default, null as IEnumerable<Link>, false));
            Assert.Equal(BlankSpan.Instance, proxyTracer.StartRootSpan("foo", SpanKind.Client, default, null as Func<IEnumerable<Link>>, false));
            ...
            Assert.Equal(BlankSpan.Instance, proxyTracer.StartSpan("foo", SpanKind.Client, default, null as IEnumerable<Link>, false));
            Assert.Equal(BlankSpan.Instance, proxyTracer.StartSpan("foo", SpanKind.Client, default, null as Func<IEnumerable<Link>>, false));
            ...
            Assert.Equal(BlankSpan.Instance, proxyTracer.StartSpan("foo", BlankSpan.Instance, SpanKind.Client, default, null as IEnumerable<Link>, false));
            Assert.Equal(BlankSpan.Instance, proxyTracer.StartSpan("foo", BlankSpan.Instance, SpanKind.Client, default, null as Func<IEnumerable<Link>>, false));
            ...
            Assert.Equal(BlankSpan.Instance, proxyTracer.StartSpan("foo", SpanContext.Blank, SpanKind.Client, default, null as IEnumerable<Link>, false));
            Assert.Equal(BlankSpan.Instance, proxyTracer.StartSpan("foo", SpanContext.Blank, SpanKind.Client, default, null as Func<IEnumerable<Link>>, false));
        ...
        public void ProxyTracer_UpdateTracer_StartRootSpanFrom_Kind_Timestamp_Links()
        {
            ...
            var span = (Span)proxyTracer.StartRootSpan("foo", SpanKind.Server, startTimestamp, () => new[] { new Link(linkContext) }, false);
        ...
        public void ProxyTracer_UpdateTracer_StartSpanFrom_ParentSpan_Kind_Timestamp_Links()
        {
            ...
            var linkContext = new SpanContext(ActivityTraceId.CreateRandom(), ActivitySpanId.CreateRandom(), ActivityTraceFlags.Recorded);
            var span = (Span)proxyTracer.StartSpan("child", parentSpan, SpanKind.Server, startTimestamp, () => new[] { new Link(linkContext) }, false);
        ...
        public void ProxyTracer_UpdateTracer_StartSpanFrom_ParentContext_Kind_Timestamp_Links()
        {
            ...
            var startTimestamp = PreciseTimestamp.GetUtcNow();
            var linkContext = new SpanContext(ActivityTraceId.CreateRandom(), ActivitySpanId.CreateRandom(), ActivityTraceFlags.Recorded);
            var span = (Span)proxyTracer.StartSpan("child", parentSpanContext, SpanKind.Client, startTimestamp, () => new[] { new Link(linkContext) }, false);
        ...
        public void ProxyTracer_UpdateTracer_StartSpanFrom_ImplicitParent_Kind_Timestamp_Links()
        {
            ...
            using (proxyTracer.WithSpan(parentSpan))
            {
                ...
                var span = (Span)proxyTracer.StartSpan("child", SpanKind.Consumer, startTimestamp,
                    () => new[] {new Link(linkContext)}, false);
